...
- **Newly released sets no longer fail to import until the next release** ([#141](https://github.com/StepKie/MtgCsvHelper/issues/141)). The bundled Scryfall card data is regenerated on every deploy, but deploys only ran when a release was cut — so a set released since then was missing entirely and every row from it was skipped with `No printing at <SET> #N in Scryfall data`. Reported for The Hobbit (HOB), where 77 of 694 rows dropped. The site now redeploys weekly with freshly generated card data, and can be refreshed on demand without cutting a release.
